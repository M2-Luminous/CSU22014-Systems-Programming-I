...
struct hashtable * hashtable_new(int size)
{
    ...
    return result;//result由多个table组成
}

// add an item to the hashtable
void hashtable_add(struct hashtable * this, char * item)
{
    int tmp = hash_string(item, this -> size);
    listset_add(this->table[tmp], item);
}
...
{
    int tmp = hash_string(item, this -> size);
    return listset_lookup(this->table[tmp], item);
}
...
{
    int tmp = hash_string(item, this -> size);
    listset_remove(this->table[tmp], item);
}
